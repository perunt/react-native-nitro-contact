///
/// Contact.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2024 Marc Rousavy @ Margelo
///

#pragma once

#if __has_include(<NitroModules/JSIConverter.hpp>)
#include <NitroModules/JSIConverter.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif
#if __has_include(<NitroModules/NitroDefines.hpp>)
#include <NitroModules/NitroDefines.hpp>
#else
#error NitroModules cannot be found! Are you sure you installed NitroModules properly?
#endif

// Forward declaration of `PhoneNumber` to properly resolve imports.
namespace margelo::nitro::margeloContact { struct PhoneNumber; }
// Forward declaration of `EmailAddress` to properly resolve imports.
namespace margelo::nitro::margeloContact { struct EmailAddress; }

#include <optional>
#include <string>
#include <vector>
#include "PhoneNumber.hpp"
#include "EmailAddress.hpp"

namespace margelo::nitro::margeloContact {

  /**
   * A struct which can be represented as a JavaScript object (Contact).
   */
  struct Contact {
  public:
    std::optional<std::string> firstName     SWIFT_PRIVATE;
    std::optional<std::string> lastName     SWIFT_PRIVATE;
    std::optional<std::vector<PhoneNumber>> phoneNumbers     SWIFT_PRIVATE;
    std::optional<std::vector<EmailAddress>> emailAddresses     SWIFT_PRIVATE;
    std::optional<std::string> thumbnail     SWIFT_PRIVATE;

  public:
    explicit Contact(std::optional<std::string> firstName, std::optional<std::string> lastName, std::optional<std::vector<PhoneNumber>> phoneNumbers, std::optional<std::vector<EmailAddress>> emailAddresses, std::optional<std::string> thumbnail): firstName(firstName), lastName(lastName), phoneNumbers(phoneNumbers), emailAddresses(emailAddresses), thumbnail(thumbnail) {}
  };

} // namespace margelo::nitro::margeloContact

namespace margelo::nitro {

  using namespace margelo::nitro::margeloContact;

  // C++ Contact <> JS Contact (object)
  template <>
  struct JSIConverter<Contact> {
    static inline Contact fromJSI(jsi::Runtime& runtime, const jsi::Value& arg) {
      jsi::Object obj = arg.asObject(runtime);
      return Contact(
        JSIConverter<std::optional<std::string>>::fromJSI(runtime, obj.getProperty(runtime, "firstName")),
        JSIConverter<std::optional<std::string>>::fromJSI(runtime, obj.getProperty(runtime, "lastName")),
        JSIConverter<std::optional<std::vector<PhoneNumber>>>::fromJSI(runtime, obj.getProperty(runtime, "phoneNumbers")),
        JSIConverter<std::optional<std::vector<EmailAddress>>>::fromJSI(runtime, obj.getProperty(runtime, "emailAddresses")),
        JSIConverter<std::optional<std::string>>::fromJSI(runtime, obj.getProperty(runtime, "thumbnail"))
      );
    }
    static inline jsi::Value toJSI(jsi::Runtime& runtime, const Contact& arg) {
      jsi::Object obj(runtime);
      obj.setProperty(runtime, "firstName", JSIConverter<std::optional<std::string>>::toJSI(runtime, arg.firstName));
      obj.setProperty(runtime, "lastName", JSIConverter<std::optional<std::string>>::toJSI(runtime, arg.lastName));
      obj.setProperty(runtime, "phoneNumbers", JSIConverter<std::optional<std::vector<PhoneNumber>>>::toJSI(runtime, arg.phoneNumbers));
      obj.setProperty(runtime, "emailAddresses", JSIConverter<std::optional<std::vector<EmailAddress>>>::toJSI(runtime, arg.emailAddresses));
      obj.setProperty(runtime, "thumbnail", JSIConverter<std::optional<std::string>>::toJSI(runtime, arg.thumbnail));
      return obj;
    }
    static inline bool canConvert(jsi::Runtime& runtime, const jsi::Value& value) {
      if (!value.isObject()) {
        return false;
      }
      jsi::Object obj = value.getObject(runtime);
      if (!JSIConverter<std::optional<std::string>>::canConvert(runtime, obj.getProperty(runtime, "firstName"))) return false;
      if (!JSIConverter<std::optional<std::string>>::canConvert(runtime, obj.getProperty(runtime, "lastName"))) return false;
      if (!JSIConverter<std::optional<std::vector<PhoneNumber>>>::canConvert(runtime, obj.getProperty(runtime, "phoneNumbers"))) return false;
      if (!JSIConverter<std::optional<std::vector<EmailAddress>>>::canConvert(runtime, obj.getProperty(runtime, "emailAddresses"))) return false;
      if (!JSIConverter<std::optional<std::string>>::canConvert(runtime, obj.getProperty(runtime, "thumbnail"))) return false;
      return true;
    }
  };

} // namespace margelo::nitro
